  ...
  useEffect(() => {
    const fetchDiagnoses = async () => {
      ...
      const { data, error } = await supabase
        .from('diagnoses')
        .select('id, session_id, created_at, crop, model_name, cost_usd, primary_diagnosis, primary_common_name, primary_confidence, secondary_diagnosis, secondary_common_name, secondary_confidence, treatment_shown, treatment_group, pre_treatment_advice, expert_validation, expert_correct_diagnosis, photo_1_url, photo_2_url, reasoning, short_note, follow_up_suggestion, expert_notes, treatment_validation, expert_correct_treatment, status, treatment_groups(name, active_ingredients), vegclinic_user_id, vegclinic_users(first_name, last_name)')
